...
- Hero title `1:567`: `Трезвые / перевозчики / в Макс`
...
- Hero title `1:479`: `Трезвые перевозчики в Макс` (без слешей; в HTML строки разделяются через `<br>`, не символами `/`)
...
- `1:479` title node metrics directly: `343 x 132`, `Unbounded Bold 40`, `44px` line-height, centered; raw text `"Трезвые перевозчики в Макс"` — без слешей; в HTML строки через `<br>`
